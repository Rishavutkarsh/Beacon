...
OUT_DIR = Path("/kaggle/working/beacon_real_problem_cpt_generation_v1")
RESULT_PATH = OUT_DIR / "generation_result.json"
EXPECTED_ROWS = 60
EXPECTED_EVAL_HASH = "0ea8db90e76571f09ef048d1e2e4c6204eba8638f2b2a6a21eafc9fce9e3ce6d"
MAX_SEQ_LENGTH = 1024
MAX_NEW_TOKENS = 220
PINNED = [
    "unsloth==2026.5.2",
...
ADAPTER_CANDIDATES = [
    ...
    Path("/kaggle/input/notebooks/rishavutkarsh/beacon-dapt-cpt-train-v1/beacon_dapt_cpt_v1/adapter_best_dev"),
]
...
def resolve_adapter() -> Path:
    ...
    if input_root.exists():
        for config_path in sorted(input_root.rglob("adapter_config.json")):
            ...
            if "adapter_best_dev" in text and "beacon" in text:
                return parent
    visible = [str(path) for path in sorted(input_root.iterdir())] if input_root.exists() else []
    raise RuntimeError(f"Could not resolve CPT adapter_best_dev; visible_inputs={visible}")
...
def main() -> None:
    ...
    try:
        ...
        eval_path = data_dir / "beacon_real_problem_eval_v1.jsonl"
        ...
        result.update({"data_dir": str(data_dir), "model_path": str(model_path), "adapter_path": str(adapter_path), "eval_hash": eval_hash, "row_count": len(rows)})
        write_json(RESULT_PATH, result)
        ...
        for index, item in enumerate(rows, start=1):
            ...
            generations.append({
                ...
                "prompt_sha256": hashlib.sha256(item["user_prompt"].encode("utf-8")).hexdigest(),
                "response": answer,
            ...
            print(f"[beacon-cpt-generation] {index}/{len(rows)} {item['example_id']}", flush=True)
        write_jsonl(OUT_DIR / "cpt_best_dev_generations.jsonl", generations)
        result["status"] = "success"
        result["stage"] = "complete"
        result["outputs"] = {"cpt_best_dev_generations": str(OUT_DIR / "cpt_best_dev_generations.jsonl")}
        result["generation_hash"] = sha256_file(OUT_DIR / "cpt_best_dev_generations.jsonl")
        write_json(RESULT_PATH, result)
