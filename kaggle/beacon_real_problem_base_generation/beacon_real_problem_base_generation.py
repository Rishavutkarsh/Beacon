...
OUT_DIR = Path("/kaggle/working/beacon_real_problem_base_generation_v1")
...
def main() -> None:
    ...
    try:
        ...
        result.update({"data_dir": str(data_dir), "model_path": str(model_path), "eval_hash": eval_hash, "row_count": len(rows)})
        write_json(RESULT_PATH, result)
        ...
        for index, item in enumerate(rows, start=1):
            ...
            print(f"[beacon-base-generation] {index}/{len(rows)} {item['example_id']}", flush=True)
        write_jsonl(OUT_DIR / "base_gemma_generations.jsonl", generations)
        result["status"] = "success"
        result["stage"] = "complete"
        result["outputs"] = {"base_gemma_generations": str(OUT_DIR / "base_gemma_generations.jsonl")}
        result["generation_hash"] = sha256_file(OUT_DIR / "base_gemma_generations.jsonl")
        write_json(RESULT_PATH, result)
