...
OUT_DIR = Path("/kaggle/working/beacon_source_qa_cpt_generation_v1")
...
def main() -> None:
    ...
    try:
        ...
        adapter_path = resolve(ADAPTER_CANDIDATES, "adapter_config.json", "CPT adapter")
        ...
        result["stage"] = "generate_cpt"
        all_generations = generate_candidate("cpt_best_dev", rows=rows, model_path=model_path, adapter_path=adapter_path)
        write_jsonl(OUT_DIR / "cpt_source_qa_generations.jsonl", all_generations)

        result["status"] = "success"
        result["stage"] = "complete"
        result["outputs"] = {"cpt_source_qa_generations": str(OUT_DIR / "cpt_source_qa_generations.jsonl")}
        result["generation_hash"] = sha256_file(OUT_DIR / "cpt_source_qa_generations.jsonl")
        result["candidate_counts"] = {"cpt_best_dev": EXPECTED_ROWS}
        write_json(RESULT_PATH, result)
