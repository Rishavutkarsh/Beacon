...
OUT_DIR = Path("/kaggle/working/beacon_source_qa_qwen_base_judge_v1")
...
def summarize(scores: list[dict[str, Any]], invalid_count: int) -> dict[str, Any]:
    ...
    for candidate in sorted({row["candidate_id"] for row in scores}):
    ...
    for row in scores:
        if not row["parse_ok"]:
            continue
        exposure = "train_seen" if row["seen_in_cpt_train"] else "heldout"
        exposure_breakdown[exposure][row["verdict"]] += 1
        if row["unsafe_extra"]:
            exposure_breakdown[exposure]["unsafe_extra"] += 1
        if row["unsupported_extra"]:
            exposure_breakdown[exposure]["unsupported_extra"] += 1
    return {
        "status": "success",
        "stage": "complete",
        ...
        "decision_note": "Base-only source-QA judge run. Use this to establish the domain-knowledge baseline; paired CPT comparison should run after CPT generations exist.",
    }
...
def main() -> None:
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError(
                f"Expected eval={EXPECTED_ROWS}, base={EXPECTED_GENERATIONS_PER_CANDIDATE}; "
                f"got eval={len(eval_rows)}, base={len(base_generations)}"
            )
        generations = base_generations
        eval_by_id = {row["example_id"]: row for row in eval_rows}
        result.update({"data_dir": str(data_dir), "base_generation_dir": str(base_gen_dir), "judge_model_path": str(judge_model_path), "eval_hash": eval_hash})
        write_json(RESULT_PATH, result)
