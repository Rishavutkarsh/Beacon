...
OUT_DIR = Path("/kaggle/working/beacon_tool_dpo_from_cpt_trl_v1")
...
CPT_ADAPTER_PATH_CANDIDATES = [
    Path("/kaggle/input/beacon-dapt-cpt-v2-checkpoints/checkpoint-300"),
    Path("/kaggle/input/datasets/rishavutkarsh/beacon-dapt-cpt-v2-checkpoints/checkpoint-300"),
    Path("/kaggle/input/beacon-dapt-cpt-v2-checkpoints/trainer/checkpoint-300"),
    Path("/kaggle/input/datasets/rishavutkarsh/beacon-dapt-cpt-v2-checkpoints/trainer/checkpoint-300"),
    Path("/kaggle/input/beacon-dapt-cpt-v2-checkpoints"),
    Path("/kaggle/input/datasets/rishavutkarsh/beacon-dapt-cpt-v2-checkpoints"),
]
PINNED = [
    "unsloth==2026.5.2",
...
def saved_checkpoint_steps(trainer_dir: Path) -> list[int]:
    return sorted(
...
def main() -> None:
    ...
    result: dict[str, Any] = {
        "status": "started",
        "stage": "init",
        "training_intent": "trl_light_dpo_tool_decision_steering_directly_from_cpt",
        "research_basis": {
        ...
        "drift_controls": {
            "base_adapter": "beacon-dapt-cpt-v2-checkpoints/checkpoint-300",
    ...
    try:
        ...
        adapter_path = resolve(CPT_ADAPTER_PATH_CANDIDATES, "adapter_config.json", "CPT checkpoint adapter")
        ...
        update_result(stage="load_cpt_adapter")
        ...
        saved_steps = saved_checkpoint_steps(OUT_DIR / "trainer")
        ...
        result.update({
            ...
            "lineage": "Gemma 4 E2B IT -> Beacon CPT checkpoint-300 -> DPO on tool-use preference pairs",
        })
        write_json(RESULT_PATH, result)
    except Exception as exc:
        saved_steps = saved_checkpoint_steps(OUT_DIR / "trainer")
        salvage: dict[str, Any] = {"saved_steps": saved_steps}
        if saved_steps:
            latest_step = saved_steps[-1]
            latest_checkpoint = OUT_DIR / "trainer" / f"checkpoint-{latest_step}"
            salvage["latest_step"] = latest_step
            salvage["latest_checkpoint_copy"] = copy_checkpoint(latest_checkpoint, OUT_DIR / "adapter_best_dev_on_error")
        result.update({"status": "error", "error": repr(exc), "traceback": traceback.format_exc(), "salvage": salvage})
        write_json(RESULT_PATH, result)
        raise
