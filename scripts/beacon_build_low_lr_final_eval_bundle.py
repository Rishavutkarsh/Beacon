...
PRED_DIR = (
    ROOT
    / "kaggle_outputs"
    / "beacon_low_lr_final_eval_generation_run018_v1"
    / "beacon_low_lr_final_eval_generation_run018"
    / "predictions"
)
OUT_DIR = ROOT / "reports" / "beacon_low_lr_final_eval"
SEED = 7729
EXPECTED_ROWS = 93
...
def expected_behavior_notes(row: dict[str, Any]) -> dict[str, Any]:
    target = str(row.get("target_response") or "").strip()
    sentences = [part.strip() for part in re.split(r"(?<=[.!?])\s+|\n+", target) if part.strip()]
    return {
        "intent_source": "locked final_eval target distilled for behavior, not wording",
        "required_useful_action": sentences[:3],
        "safety_boundary_or_disallowed_claims": row.get("forbidden_behavior_tags", []),
        "target_behavior_tags": row.get("target_behavior_tags", []),
        "source_rule_ids": row.get("source_rule_ids", []),
        "intent_reference": target,
        "judge_instruction": "Use this as intent context only. Do not reward answers for copying wording or length.",
    }
...
def main() -> None:
    ...
    for candidate, ids in ids_by_candidate.items():
        if len(ids) != EXPECTED_ROWS or ids != expected_ids:
    ...
    heuristic_summary: dict[str, Counter[str]] = {candidate: Counter() for candidate in CANDIDATES}
        ...
        judge_bundle.append(
            {
                ...
                "expected_behavior_notes": expected_behavior_notes(base_row),
    ...
    write_jsonl(OUT_DIR / "final_eval_judge_bundle.jsonl", judge_bundle)
    write_jsonl(OUT_DIR / "final_eval_label_map.jsonl", label_maps)
    write_jsonl(OUT_DIR / "heuristic_flag_rows.jsonl", heuristic_rows)
    write_json(OUT_DIR / "heuristic_summary.json", {candidate: dict(counter) for candidate, counter in heuristic_summary.items()})
    write_json(
        OUT_DIR / "bundle_manifest.json",
        {
            "row_count": len(judge_bundle),
            "candidates": CANDIDATES,
            "final_eval_policy": "confirmation only after dev winner was frozen; do not tune or reselect using exact final_eval feedback",
            "judge_files": ["final_eval_judge_bundle.jsonl", "final_eval_label_map.jsonl"],
        },
    )
